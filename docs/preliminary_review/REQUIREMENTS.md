...
作成日：2026-05-14
最終更新：2026-06-24（§0 を追加し PoC 決定を反映）
対象：事前レビュー機能追加
...
- **提出タイミング（計画/実績）で「検索シート」と「レビュー列」を分岐**（転記結果 MRC1 の C8＝計画実績区分で判定）：
  ...
  - **「情報提供時」は別ナレッジのため検索対象外。**
  - **実績の差分レビューは Tool5（`detect_plan_diff`）**で実施（計画G vs 実績K の乖離が大きい時のみ指摘）。
    数値項目は MRC2 へ移管済みのため **MRC2 側で比較**（バックログ課題①「Tool5→MRC2」と連動）。
  - 共通の識別項目（件名・費目・号炉等）は区分に関わらず確認対象。
  - 実装：区分→`submission_timing` の後段フィルタ＋レビュー対象セルの G/K 選別（config駆動・**I/F不変**・measure-first）。

### 0-4. データストアの役割（DB設計）
- **Firestore**＝レビュー中の運用状態（セッション・指摘・採否のリアルタイム・undo・履歴）。**PoCはこれのみ**。
- **Agent Search(F3)**＝検索で根拠を引く本体。承諾ナレッジを次回に効かせる**還流先**もここ（将来）。
- **BigQuery**＝承諾/棄却の**分析・蓄積**（検索はしない）。**将来**、Firestore→BigQuery（ゼロ移行）で追加。
...
- 図解フロー（PoC現状/将来）：`RAG_VERIFICATION.md §15`（Mermaid）を参照。

### 0-6. 決定済み・未実装（実装バックログ）
- 数値の妥当性チェック（決定論・**単位は円・許容0**）／転記の**粒度感**チェック（観点）／
  config 移行の転記系パス追従／Ranking API 実装 等は **決定済み・未実装**。詳細は `RAG_VERIFICATION.md §2/§12`。
