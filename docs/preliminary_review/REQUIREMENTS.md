...
- **BigQuery＝2用途（混同しないこと）**：
  - **① F2/F3知識のデータ置き場**（平坦テーブル）。**PoCで採用**し、**Agent Search がこれを索引**して検索する（§0-7／`RAG_VERIFICATION.md §3-3`）。F2/F3とも実装済み（2026-07-03）。
...
> `Excel(正本) → 平坦化(ver5.3) → BigQuery → Agent Search索引`（F3=nuro-f3-bq-knowledge／F2=nuro-f2-bq-knowledge）。
> 旧 Excel→Agent Search 直接投入（nuro-f3-knowledge／nuro-f2-knowledge）は比較基準として残置・検索には未使用
> （`RAG_VERIFICATION.md §1-10〜13`）。
>
> 用語：**平坦化**＝「KNI_*の横持ち（やりとりが横に伸びる）」を「**1メッセージ＝1行**」の縦持ち
> （出力用シート相当）に変換すること。**正本**＝編集の起点（本物のコピー）。

- **データ形式（R1）**：F2/F3ナレッジは新様式 ver.5.3 の**平坦形式（1メッセージ＝1行）**で構造化。
  （F3列は下記。F2列は業務カテゴリ/From/To/優先度/ステータス/タイトル/事象概要/判断基準/対応結果等＝`_excel_reader.F2_VER53_COLUMNS`）
