...
uv run python scripts/generate_supplement_captions.py --dry-run # 画像抽出のみ確認
```
...
```bash
uv run python scripts/ingest_knowledge.py --target supplement
```
...
uv run python scripts/generate_supplement_captions.py
