...
- **Firestore**＝レビュー中の運用状態（リアルタイム・PoCはこれのみ）。
- **Agent Search(F3)**＝検索で根拠を引く本体／承諾ナレッジの還流先（将来）。
- **BigQuery**＝承諾/棄却の分析・蓄積（検索はしない・将来）。
...
| # | 項目 | 状態 | メモ |
|---|---|---|---|
| ① | Tool5 数値差分のMRC1対象消失 | ✅ | 現設計に整合（§1-4）。MRC2に計画/実績ペア定義で自動対象化 |
...
- **移行トリガー（要件§4-5）**：F3>1万件 ／ 棄却率50%超 → Reranking有効化・Agentic RAG本格化・DB化。
