...
レビューエージェント（`agent.py`）の実行基盤を Google ADK 2.0 に移行した。
...
ループ終了条件・最大試行回数・並列 Tool 呼び出しの制御・エラーリカバリが全て `agent.py` に集まる。

**② Phase 3 Sub Agent 追加時に爆発する**

マルチモーダル Sub Agent は「自分で画像を解釈して返す」独自の ReAct ループを持つ。
親子間の状態受け渡し・エラー伝播・並列実行を手書きすると `agent.py` が事実上のフレームワークになる。
...
POST /api/review
      │
      ▼
agent.run_review()  ← Langfuse @observe でトレース
      │
      ▼
...
├── agent.py               ← run_review() エントリーポイント（外部 I/F は Phase1 から不変）
├── config.py              ← レビュー固有設定（Vertex Search / Reranking / BigQuery）
├── models.py              ← ReviewItem（api/models.py から re-export）
├── review_logic.py        ← 純粋なロジック関数群（循環 import を防ぐ中立モジュール）
├── criteria_loader.py     ← Gemini system instruction 生成
├── workflow/              ← ADK Workflow（レビューの実行フロー）
│   ├── nodes.py           ← FunctionNode 関数定義（Tool1〜5 + SynthesisNode）
...
└── knowledge/             ← F2/F3ナレッジへのアクセス
    ...
    ├── excel_reader.py    ← F2/F3ナレッジExcel読み込み
    └── result_reader.py   ← 転記結果の復元・クエリ文脈導出
```
...
### 4-1. `review_logic.py` を分離した理由（循環 import 対策）

ADK 移行前は `agent.py` に全ロジックが集約されていた。
`workflow/nodes.py` から `agent.py` を import すると循環 import が発生するため、
共有ロジック（`detect_plan_diff`, `_build_prompt`, `_parse_review_response` 等）を
`review_logic.py` に抽出して依存関係を一方向にした。

```
agent.py  ──┐
                      ├── import ──► review_logic.py
workflow/nodes.py      ──┘
```

`agent.py` は後方互換のため `review_logic.py` から re-export している。
...
`agent.py`・API エンドポイント・フロントエンドへの変更は不要。
...
E2E テスト（`test_review_e2e.py`）では `workflow/nodes.py` の `call_gemini` を
