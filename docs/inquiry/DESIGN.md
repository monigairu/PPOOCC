...
- **バックログ（改善候補・計測根拠付き）**：F3は1行=1メッセージ（D-9）のため、検索が会話の
  片側（NuRO確認のみ等）しか返さないケースがある（A-4 はヒット1件のみ）。同一案件IDの
  往復メッセージ（確認・回答の対）を②③の文脈に補完取得する small-to-big retrieval は、
  ②の判定材料を構造的に増やせる（採否は評価セット拡充後に判断）。
