...
**フェーズ4追補：検証マトリクス評価（D-21/D-22・2026-07-17）**：
- 報告用の検証マトリクス（検索精度×回答品質の難易度別）に合わせ、`qa_cases.yaml` に
  難易度タグ（`search_level`/`gen_level`）と5問を追加（計16問）：A-6/A-7＝A-1/A-3 と同じ
  正解レコードの**言い換え版**（語彙の影響のみ分離）・A-8/A-9＝**少数費目**（維持管理費・処分費）・
  A-10＝類似レコード群からの**選別**。`eval_inquiry.py` に難易度別集計を追加（判定ロジック不変）。
- 初回計測で A-8 が④接地検査で誤棄却（3run中1回・安全側）→プローブで要素切り分け：
  回答1文目が質問の話題句（「維持管理費の定期点検について、」）をなぞると、本文のみの fact では
  **話題句の有無だけで score 0.98⇔0.22 が反転**（句点前空白・「これに対応するため、」は無関係）
  → **D-22（案件メタ行）を導入**し fact 側に費目・工事名等の語彙を持たせて解消。
- **D-22 導入後（16問×3run）**：マトリクス全セルPASS。検索=3難易度とも全問TOP1ヒット・
  生成=4難易度とも100%（正答率30/30・引用正確性35/35・棄却率18/18・**誤答率0/18**）。
  誤棄却0/30。B群ハードネガティブの棄却経路も不変（0件即棄却12・②ゲート6）＝安全側は不変。
- 接地スコアは 0.78〜0.99（最低は A-1 の run 振れ）＝閾値0.6 の下側マージン 0.18。
  応答時間 平均11.9s/最大32.7s（D-19 計測時より増。§9-6 未確定のまま要注視）。
...
| D-21 | ④に渡す回答から**文頭の接続詞**（また・さらに・なお・加えて・そのほか・その他）を機械的に除去する（`_normalize_for_claims`。表示用の回答本文は不変） | プローブ実測：「また、〜が求められます」は fact にほぼ同文があっても支持なしと判定され score 0.63→除去で 0.86。文頭接続詞は除去しても主張の内容が変わらないため、検査入力からのみ取り除く（D-20 と同系統の入力整形） | 確定（計測で問題があれば差し戻し） |
| D-22 | ④の fact 先頭に**案件メタ行**（費目・工事名・提出タイミング・対象。空・「－」は省略）を付与する（`_build_case_facts`） | 検証マトリクス拡充（2026-07-17）で発見：質問は本文でなく**案件属性の語彙**（「維持管理費の定期点検」＝費目・工事名）で書かれることが多く、回答1文目が質問の話題語をなぞると本文だけの fact では**話題句の有無だけで score 0.98⇔0.22 が反転**（A-8 プローブで要素切り分け済み。生成の run 間振れで話題語入り変種が出た回のみ誤棄却＝3run中1回）。メタ行は実データ由来のため、**誤った話題（違う費目等）をなぞった回答は引き続き支持されない**＝安全側を緩めない。付与後 A-8 は 3/3 正答（接地0.98） | 確定（計測で問題があれば差し戻し） |
