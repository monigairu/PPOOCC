...
本書は、転記系（様式自動作成1 / N対1）について、読み手が「何をしている処理か」「どのデータが次に渡るか」「どこで人間確認（HITL）が入るか」を追えるようにまとめる。

**事前レビュー系（RAG）は別システムのため、アーキテクチャを [`docs/preliminary_review/ARCHITECTURE.md`](preliminary_review/ARCHITECTURE.md) にファイル分割した。**
...
1. 転記系（様式自動作成1 / N対1）－ 複数資料 → MRC1/MRC2 様式 Excel（本書）
2. 事前レビュー系（NuRO 向け）－ 転記済み mappings → AI 指摘（ReviewItem）（→ 別ファイル）

読み方の目安：

- 全体の流れだけ把握したい場合は、この章と A-1 の Mermaid 図を見る。
- 転記で止まった理由を調べる場合は A-7 の HITL を見る。
- どの関数を直せばよいか探す場合は A-2 の関数表を見る。
- dict の中身を確認したい場合だけ、後半の「データ構造リファレンス」を見る。
- 事前レビュー（RAG）の流れを知りたい場合は `preliminary_review/ARCHITECTURE.md` を見る。
...
| 23 | 補助処理 | `_read_excel_by_schema()` (`preliminary_review/knowledge/excel_reader.py`) | 1 schema + 1 workbook | 検索投入向けレコード配列：人間が作った過去のExcel（ナレッジ等）を読み込み、指定された様式（スキーマ）に自動で並べ替えて、AIが検索で使いやすいレコードの配列に変換する翻訳リーダー。 |
...
# B. 事前レビュー系（RAG）→ 別ファイルへ分割

事前レビュー系（RAG）は様式自動作成とは別システムのため、アーキテクチャを
[`docs/preliminary_review/ARCHITECTURE.md`](preliminary_review/ARCHITECTURE.md) に分割した（2026-07-03）。
本書は以降、転記系（様式自動作成）のみを扱う。
...
- `sessions/{session_id}/review_results`：事前レビュー系が書き込むAI指摘・採否（構造は `preliminary_review/ARCHITECTURE.md` B-7 を参照）
...
- 事前レビュー系（RAG）の制約・バックログは `preliminary_review/ARCHITECTURE.md` B-9 および `RAG_VERIFICATION.md` §2 を参照