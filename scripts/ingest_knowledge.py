...
実行方法:
    ...
    uv run python scripts/ingest_knowledge.py --target f3 --backend bigquery
        # F3を ver5.3 平坦形式で BigQuery（データ置き場）へロードする（§0-7 R3）
        # Excel(正本) → 平坦化 → BigQuery → Agent Search索引（create_datastores.py）
...
def ingest(target: str, backend: str = "direct") -> None:
    ...
    backend="bigquery": F3のみ。ver5.3平坦形式で BigQuery（データ置き場）へロード（§0-7 R3）
                        Agent Search の索引作成は create_datastores.py 側で行う
    """
    ...
    if backend == "bigquery":
        if target not in ("f3", "all"):
            print(f"エラー: --backend bigquery は F3 専用です（--target {target} は未対応）")
            sys.exit(1)
        _ingest_f3_bigquery()
        return
...
def _ingest_f3_bigquery() -> None:
    """F3ナレッジを ver5.3 平坦形式（1メッセージ=1行）で BigQuery へロードする。

    データの流れ（REQUIREMENTS §0-7 R3 / RAG_VERIFICATION §3-3）：
        Excel(正本) → 平坦化(read_all_f3 + to_ver53_rows) → BigQuery(データ置き場)
    ...
    print("── F3ナレッジ BigQuery ロード（ver5.3平坦形式）─────────")
    records = read_all_f3()
    print(f"  取得: {len(records)} 件")
    if not records:
    ...
    table_id = f"{GCP_PROJECT_ID}.{BIGQUERY_DATASET_ID}.{BIGQUERY_F3_TABLE_ID}"
    schema = [
        bigquery.SchemaField(name, "INT64" if name == "round" else "STRING")
        for name in VER53_COLUMNS + VER53_AUX_COLUMNS
    ]
